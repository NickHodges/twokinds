...
export function getUserIdFromSession(session: Session | ExtendedSession | null): number | null {
  ...
  try {
    ...
    // ALWAYS prioritize the dbId if available from middleware
    if (session.user.dbId !== undefined && session.user.dbId !== null) {
      // Convert to number if it's a string
      if (typeof session.user.dbId === 'string') {
        const numericDbId = parseInt(session.user.dbId, 10);
        if (!isNaN(numericDbId)) {
          logger.debug('Converted string dbId to number:', numericDbId);
          return numericDbId;
        }
      } else if (typeof session.user.dbId === 'number') {
        // Already a number
        logger.debug('Using numeric dbId from session:', session.user.dbId);
    ...
    if (session.user.email) {
      logger.debug('Looking up user ID by email:', session.user.email);
      return getUserDbId(session.user).catch((err) => {
        logger.error('Error looking up user by email:', err);
        return null;
      });
    }

    // Try parsing the session ID as a number as a last resort
    if (session.user.id) {
      const numericId = Number(session.user.id);
      if (Number.isFinite(numericId)) {
        logger.debug('Using parsed numeric ID from session.user.id:', numericId);
        return numericId;
      }
    }

    logger.error('Could not determine numeric user ID from session');
    return null;
  } catch (error) {
    logger.error('Error in getUserIdFromSession:', error);
    return null;
  }
}
