...
export const POST: APIRoute = async ({ request, _redirect }) => {
  try {
    ...
    if (!session?.user?.id) {
      return new Response(JSON.stringify({ error: 'You must be logged in to delete a saying' }), {
        status: 401,
      });
    }
    ...
    if (!sayingId || typeof sayingId !== 'number') {
      return new Response(JSON.stringify({ error: 'Invalid saying ID' }), { status: 400 });
    }
    ...
    return new Response(JSON.stringify({ error: 'Failed to delete saying' }), { status: 500 });
  }
};
